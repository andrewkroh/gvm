...
`FOR /f "tokens=*" %i IN ('"gvm.exe" 1.26.5') DO %i`

powershell:

`gvm --format=powershell 1.26.5 | Invoke-Expression`
...
Use `gvm` with fish shell by executing `gvm 1.26.5 | source` in lieu of using `eval`.
