...
`FOR /f "tokens=*" %i IN ('"gvm.exe" 1.26.6') DO %i`

powershell:

`gvm --format=powershell 1.26.6 | Invoke-Expression`
...
Use `gvm` with fish shell by executing `gvm 1.26.6 | source` in lieu of using `eval`.
