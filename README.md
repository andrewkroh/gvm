...
`FOR /f "tokens=*" %i IN ('"gvm.exe" 1.25.2') DO %i`

powershell:

`gvm --format=powershell 1.25.2 | Invoke-Expression`
...
Use `gvm` with fish shell by executing `gvm 1.25.2 | source` in lieu of using `eval`.
