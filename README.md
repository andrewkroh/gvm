...
`FOR /f "tokens=*" %i IN ('"gvm.exe" 1.26.3') DO %i`

powershell:

`gvm --format=powershell 1.26.3 | Invoke-Expression`
...
Use `gvm` with fish shell by executing `gvm 1.26.3 | source` in lieu of using `eval`.
